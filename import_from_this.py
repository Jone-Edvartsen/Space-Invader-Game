...
#from shooting_game_pt_2 import second_game_main
import pygame
import sys
import time
import random
...
class Player:
    def __init__(self, x, y, color, TotalScore):
        self.x = x
        self.y = y
        self.color = color
        self.height = 50
...
# Explosion class (when meteor and alien boss is hit)
class Explosion:
    def __init__(self, x, y, size, images): #Get last_image_time from time.time() inside the 
        self.x = x
        self.y = y
        self.size = size
    ...
    def update(self, frame_duration):
        ...
        if current_time - self.last_image_time >= frame_duration:
            self.image_index += 1
...
def show_small_winner_message(screen, player1, player2, winner):
    ...
    player1_total_points_all_games = player1_total_points + player1.TotalScore
    player2_total_points_all_games = player2_total_points + player2.TotalScore
    ...
    # Display scoreboard
    scoreboard_title = "Scoreboard after 2 games:"
    scoreboard_title_surface = comic_sans_font.render(scoreboard_title, True, WHITE)
    title_x = (WIDTH - scoreboard_title_surface.get_width()) // 2
    title_y = HEIGHT // 2 - 150
    
    screen.blit(scoreboard_title_surface, (title_x, title_y))
    
    # Display ..........
    dots_title = "..............................................."
    dots_title_surface = comic_sans_font.render(dots_title, True, WHITE)
    title_yy = title_y + 30
    screen.blit(dots_title_surface, (title_x, title_yy))
    ...
    for i, (player, points) in enumerate(ranked_players):
        ...
        if player == 1:
            player_label_surface = comic_sans_font.render(player_label, True, RED)
        else:
            player_label_surface = comic_sans_font.render(player_label, True, GREEN) 
        label_x = (WIDTH - player_label_surface.get_width()) // 2
        
        label_y = title_yy + 40 + i * 30

        screen.blit(player_label_surface, (label_x, label_y))
...
def check_edges_touching(player, alien_boss):
    p_left = player.x
    p_right = player.x + player.width
    p_top = player.y
    p_bottom = player.y + player.height
    
    ab_left = alien_boss.x
    ab_right = alien_boss.x + alien_boss.width
    ab_top = alien_boss.y
    ab_bottom = alien_boss.y + alien_boss.height
    
    touching_left_edge = (p_left == ab_right or p_right == ab_left)
    touching_right_edge = (p_right == ab_left or p_left == ab_right)
    touching_top_edge = (p_top == ab_bottom or p_bottom == ab_top)
    touching_bottom_edge = (p_bottom == ab_top or p_top == ab_bottom)

    return touching_left_edge or touching_right_edge or touching_top_edge or touching_bottom_edge    
    
    
    
def second_game_loop(lives1, lives2, score1_pt1, score2_pt1):
    
    

    
    clock = pygame.time.Clock()
    player1 = Player(WIDTH - 40, HEIGHT // 2 - 25, RED, score1_pt1)
    player2 = Player(20, HEIGHT // 2 - 25, GREEN, score2_pt1)
    ...
    while winner is None:
        ...
        for meteor in meteors:
            ...
            if meteor.off_screen():
                ...
                if meteor_hit is not None:
                    explosion_x = meteor_hit.x + (meteor_hit.size // 2)
                    explosion_y = meteor_hit.y + (meteor_hit.size // 2)
                    explosions.append(Explosion(explosion_x, explosion_y, meteor_hit.size, explosion_images))
                    meteor_hit = None
        ...
        if alien_boss:
            ...
            if check_edges_touching(player1, alien_boss):
                player1.lives = 0
                winner = 2

            if check_edges_touching(player2, alien_boss):
                player2.lives = 0
                winner = 1 
        ...
        for explosion in explosions:
            explosion.update(frame_duration)
            if explosion.is_finished():
                finished_explosions.append(explosion)
            else:
                explosion.draw(screen)
    
        # Remove finished explosions from the list
        for finished_explosion in finished_explosions:
            explosions.remove(finished_explosion)       
...
def second_main(score1_pt1,score2_pt2):
    ...
    while True:
        player1, player2, lives1, lives2, winner = second_game_loop(lives1, lives2, score1_pt1, score2_pt2) # Update here to get winner
